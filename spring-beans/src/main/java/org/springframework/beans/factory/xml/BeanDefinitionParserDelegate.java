...
public class BeanDefinitionParserDelegate {
	 ...
	 * 解析xml文件中的bean元素
	 * 1. 提取元素中的id以及name 属性
	 * 2. 进一步解析其他所有属性，并统一封装在GenericBeanDefinition 类型的实例中。
	 * 3. 如果检测到bean 没有指定 beanName，那么使用默认规则为此bean 生成beanName
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, BeanDefinition containingBean) {
		...
		if (beanDefinition != null) {
			if (!StringUtils.hasText(beanName)) {
				try {
					if (containingBean != null) {
						//如果不存在 beanName，那么根据Spring 中提供的命名规则为当前bean生成对应的beanName
	 ...
	 * May return <code>null</code> if problems occurred during the parsing of the bean definition.
	 *
	 * 解析xml文件中的bean元素的具体逻辑
	 *
	 */
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			...
			//创建用于继承属性的AbstractBeanDefinition 类型的 GenericBeanDefinition
	...
	public AbstractBeanDefinition parseBeanDefinitionAttributes(Element ele, String beanName,
		...
		if (ele.hasAttribute(SCOPE_ATTRIBUTE)) {
			...
			if (ele.hasAttribute(SINGLETON_ATTRIBUTE)) {
				//scope属性和singleton属性只能二选一，不能同时出现
		...
		//解析singleton属性
			...
			//在嵌入beanDefinition 情况下，且没有单独指定scope情况下，则使用父类默认的属性
	 ...
	 *
	 * @param ele
	 * @param containingBd 父类的bean, 对顶层元素的解析, 应设置为null
	 * @return
	 */
	public BeanDefinition parseCustomElement(Element ele, BeanDefinition containingBd) {
		...
		//用自定义的NamespaceHandler 开始解析
		return handler.parse(ele, new ParserContext(this.readerContext, this, containingBd));
	}

	/**
	 * 处理bean的属性有自定义标签，
	 * <bean id="test" class="com.lin.TestUser">
	 *     <mybean:user username="zhangsan"/>
	 * </bean>
	 *
	 * 当spring中 bean 使用的是默认标签配置，但是其中的子元素却使用了自定义的配置时，下列方法起作用
	 *
	 * @param ele
	 ...
	 * @param ele
	 * @param definitionHolder
	 * @param containingBd 父类bean，其作用是为了使用父类的scope属性，以备若子类没有scope属性时，默认使用父类的scope属性
	 * @return
	 */
	...
	/**
	 * 根据xml元素找到对应的NamespaceHandler，然后进行处理
	 *
	 * @param node
	 * @param originalDef
	 * @param containingBd
	...
	/**
	 * 获取命名空间，然后和spring固定的命名空间(http://www.springframework.org/schema/beans)做对比，
	 * 如果相等，则是默认的命名空间； 如果不相等，则是自定义的命名空间
	 * @param node
	 * @return
	 */
